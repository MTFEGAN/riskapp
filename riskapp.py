...
PRIMARY_COLOR = "#1f77b4"  # Customize as needed

# Define JavaScript for AgGrid styling
...
def load_historical_data(excel_file):
    try:
        # Load only necessary sheets and columns if possible
        excel = pd.ExcelFile(excel_file)
        df_list = []
        for sheet in excel.sheet_names:
            # Optionally, specify required columns to read
...
def process_yields(df):
    # Adjust yields if necessary
    for col in ['AU 3Y Future', 'AU 10Y Future']:
...
def calculate_volatilities(adjusted_price_returns, lookback_days):
    # Calculate annualized volatilities in bps
...
def calculate_covariance_matrix(adjusted_price_returns, lookback_days):
    # Calculate annualized covariance matrix in bps^2
...
# Create positions DataFrames with default values
...
with tabs[1]:
    ...
    with dm_col:
        ...
        gb_dm.configure_pagination(enabled=True, paginationPageSize=20)  # Limit to 20 rows per page
        gb_dm.configure_default_column(groupable=True, value=True, enableRowGroup=True, aggFunc='sum')
        ...
        grid_response_dm = AgGrid(
            ...
            reload_data=True,
            theme='streamlit',  # Use a lighter theme
    ...
    with em_col:
        ...
        gb_em.configure_pagination(enabled=True, paginationPageSize=20)  # Limit to 20 rows per page
        gb_em.configure_default_column(groupable=True, value=True, enableRowGroup=True, aggFunc='sum')
...
with tabs[0]:
    ...
    # Run Risk Attribution when button is clicked
    if st.button('🚀 Run Risk Attribution'):
        with st.spinner('Calculating risk attribution... This may take a few moments.'):
            ...
            # Ensure the sensitivity rate is included
            ...
            # Create expanded covariance matrix
            ...
            if missing_instruments:
                ...
                # Drop positions for missing instruments
            ...
            portfolio_volatility = np.sqrt(portfolio_variance)  # in bps

            # Compute Each Position's Stand-alone Volatility and Contribution
            expanded_volatilities = expanded_positions_vector.index.get_level_values('Instrument').map(volatilities)
            expanded_volatilities = pd.Series(expanded_volatilities.values, index=expanded_positions_vector.index)

            # Instrument Volatility per 1Y Duration in bps
            ...
            portfolio_returns = price_returns_var.dot(positions_per_instrument) * 100  # in bps
            ...
            # Risk Contributions Table using AgGrid
            ...
            # Risk Contributions by Portfolio Pie Chart
            ...
            if sensitivity_rate in price_returns_var.columns:
                ...
                if portfolio_returns_aligned.empty or sensitivity_returns_aligned.empty:
                    ...
                    if beta > 0:
                        ...
                        pl_direction = 'gain'
                        beta = -beta  # Absolute value for clarity
                    else:
                        beta_direction = 'zero'
                        yield_movement = 'no change'
                        pl_direction = 'no change'

                    # Sensitivity to a 1 bps move
                    sensitivity = beta * 1  # 1 bps
            ...
            st.subheader('📥 Download Risk Attribution Report')
    ...
    st.info('💡 Navigate to the "Input Positions" tab to enter your portfolio positions and run the risk attribution analysis.')
