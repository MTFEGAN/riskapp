...
def process_yields(df):
    # Adjust yields for AU futures if they exist
...
def calculate_returns(df):
    ...
    # Invert sign to reflect price changes
...
def main():
    ...
    # Define instruments and their portfolios
    ...
    if os.path.exists(excel_file):
        ...
        if raw_df is not None and not raw_df.empty:
            ...
            sensitivity_rate = 'US 10Y Future'
    else:
        st.sidebar.error(f"❌ Excel file '{excel_file}' not found in the app directory.")
        st.stop()
    ...
    instrument_country = {
        ...
        'US 5Y Future': 'US',
        'US 10Y Future': 'US',
        # ... map other instruments as needed ...
    }
    ...
    with tabs[1]:
        ...
        st.write("Enter positions for DM and EM portfolios below.")
    ...
    with tabs[2]:
        ...
        volatility_period = st.selectbox('Volatility lookback period:', list(volatility_period_options.keys()), index=3)
        ...
        var_period = st.selectbox('VaR lookback period:', list(var_period_options.keys()), index=1)
        ...
        if st.button('🚀 Run Risk Attribution'):
            with st.spinner('Calculating risk attribution...'):
                ...
                if df is None or df.empty:
                    st.error("No data loaded. Check the Excel file.")
                    st.stop()
                ...
                if adjusted_price_returns.empty:
                    st.warning("⚠️ No data after adjusting time zones.")
                    st.stop()
                ...
                if missing_instruments:
                    ...
                    expanded_positions_vector = expanded_positions_vector.drop(
                        labels=[(instr, pt) for instr in missing_instruments for pt in ['Outright', 'Curve', 'Spread']],
                        errors='ignore'
                    )
                    expanded_index = expanded_positions_vector.index
                    expanded_cov_matrix = pd.DataFrame(index=expanded_index, columns=expanded_index)
                ...
                metrics_col4.metric(label="📈 Daily cVaR (95%)", value=f"{cVaR_95_daily:.2f} bps" if not np.isnan(cVaR_95_daily) else "N/A")

                st.subheader('📈 Value at Risk (VaR) and Conditional VaR (cVaR)')
                st.write(f"**Daily VaR at 95%:** {VaR_95_daily:.2f} bps")
                st.write(f"**Daily cVaR at 95%:** {cVaR_95_daily:.2f} bps" if not np.isnan(cVaR_95_daily) else "**Daily cVaR at 95%:** N/A")
                st.write(f"**Daily VaR at 99%:** {VaR_99_daily:.2f} bps")
                st.write(f"**Daily cVaR at 99%:** {cVaR_99_daily:.2f} bps" if not np.isnan(cVaR_99_daily) else "**Daily cVaR at 99%:** N/A")

                # Sensitivity Analysis without OLS trendline
                if sensitivity_rate in price_returns_var.columns:
                    sensitivity_returns = price_returns_var[sensitivity_rate] * 100
                    ...
                    if portfolio_returns_aligned.empty or sensitivity_returns_aligned.empty:
                        st.warning("⚠️ Insufficient overlapping data for sensitivity analysis.")
                    else:
                        # Without OLS trendline, we just show the scatter plot
                        ...
                        st.write("No OLS trendline due to compatibility issues. If you need a regression line, consider computing regression manually.")
